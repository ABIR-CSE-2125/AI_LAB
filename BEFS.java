...
public class BEFS {
    public void bestFirstSearch(WeightedGraph wg, int[] heuristic, int start, Set<Integer> goal,
            List<List<WeightedGraph.Edge>> adj) {
        // System.out.println("Size of adjList" + adj.size());
        PriorityQueue<WeightedGraph.Node> pq = new PriorityQueue<>(new WeightedGraph.NodeComparatorBEFS());
        ...
        while (!pq.isEmpty()) {
            ...
            for (WeightedGraph.Edge edge : adj.get(u)) {
                ...
                if (!visited[v]) {
                    System.out.println("Node : " + currentNode.vertex + " h(x) : " + currentNode.heuristic);
                    int newCost = currentNode.cost + weight;
                    // System.out.println("Node : " + v + " newcost : " + newCost);
        ...
        for (Map.Entry<Integer, Integer> entry : goalCost.entrySet()) {
            System.out.println("Goal : " + entry.getKey() + " cost : " + entry.getValue());
    ...
    private void printPath(int[] parent, int goal, int gc) {
        ...
        System.out.println("Path from start to goal: " + path + " with h(x) " + 0);
    }
    ...
    public void operate() {
        ...
        bestFirstSearch(wg, heuristic, start, goal, wg.getAdjList());
    }
