...
public class Puzzle_dls {
    ...
    static boolean solveDLS(int[][] initial, int[][] fin, int maxDepth) {
        Node root = new Node(initial, 1, 2, 1, 2, 0, null);
        HashSet<String> visited = new HashSet<>();
        return DLS(root, fin, maxDepth, visited);
    }

    static boolean DLS(Node node, int[][] fin, int maxDepth, HashSet<String> visited) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (isSafe(newX, newY)) {
                ...
                if (!visited.contains(Arrays.deepToString(child.mat))) {
                    if (DLS(child, fin, maxDepth, visited))
                        return true;
                }
            }
        }
    ...
    public static void main(String args[]) {
        ...
        int maxDepth = 5; // Adjust maximum depth as needed

        if (!solveDLS(initial, fin, maxDepth)) {
